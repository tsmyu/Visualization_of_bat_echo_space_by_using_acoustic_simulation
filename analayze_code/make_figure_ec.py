...
def __make_figure(name, w, x, y, pulse_dir, echo_point_list, power_max):
    ...
    pulse_y = y_range - np.array(y) + np.array(pulse_y)
    xs = np.linspace(x[0], x[-1], len(x))
    ys = y_range - np.polyval(w, xs)
    fig = plt.figure(figsize=(12, 5))
    plt.xlim(0, x_range)
    plt.ylim(0, y_range)
    plt.xticks(np.arange(0, x_range+1, 3000))
    plt.yticks(np.arange(0, y_range+1, 1000))
    plt.xlabel("[m]")
    plt.ylabel("[m]")
    ax = fig.add_subplot(111)
    y = y_range - np.array(y)
    ...
    for i, (xx, yy, px, py) in enumerate(zip(x, y, pulse_x, pulse_y)):
        ...
        #             ax.scatter(eval(echo_point[1][i])[0], y_range-eval(echo_point[1][i])[1],
        ...
        #                        edgecolors="k")
    ax.plot((5000, 5000), (5000, 2800), 'k-', lw=4)
    ax.plot((8333, 8333), (0, 2200), 'k-', lw=4)
    ax.plot((11666, 11666), (5000, 2800), 'k-', lw=4)
    # ax.plot((5000, 5000), (5000, 2800), 'k-', lw=4, linestyle="dashed")
    # ax.plot((8333, 8333), (0, 2200), 'k-', lw=4, linestyle="dashed")
    # ax.plot((11666, 11666), (5000, 2800), 'k-', lw=4, linestyle="dashed")
...
def main(csv_list):
    ...
    # y_n, diff_tim_n, diff_power_n = __norm(
    #     flight_path_y_list, diff_tim_list, diff_power_list)
...
if __name__ == "__main__":
    ...
    if len(argvs) < 1:
        print(
            "Usage: python {} [folder of echo csv]".format(argvs[0]))
        exit()
